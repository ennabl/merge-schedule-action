...
const pullRequests = [
    ...
    {
        number: 15,
        html_url: githubPullRequestUrl(15),
        state: "open",
        body: "Simple body\n",
        head: {
            sha: "abc123pending-empty",
    ...
    {
        number: 16,
        html_url: githubPullRequestUrl(16),
        state: "open",
        body: "Simple body\n/schedule 2022-06-12",
        head: {
            sha: "abc123success",
    ...
    {
        number: 17,
        html_url: githubPullRequestUrl(17),
        state: "open",
        body: "Simple body\n/schedule 2022-06-12",
        head: {
            sha: "abc123success",
    ...
    {
        number: 18,
        html_url: githubPullRequestUrl(18),
        state: "open",
        body: "Simple body\n/schedule",
        head: {
            sha: "abc123success",
    ...
    {
        number: 19,
        html_url: githubPullRequestUrl(19),
        state: "open",
        body: "Simple body",
        head: {
            sha: "abc123success",
    ...
    {
        number: 20,
        html_url: githubPullRequestUrl(20),
        state: "open",
        body: "Simple body\n /schedule 2022-06-12",
        head: {
            sha: "abc123success",
    ...
    {
        number: 21,
        html_url: githubPullRequestUrl(21),
        state: "open",
        body: "Simple body\n",
        head: {
            sha: "abc123success",
...
const pullRequestComments = pullRequests.map((pullRequest) => {
    const bodyMappings: Record<number, string[]> = {
        ...
        17: ["<!-- Merge Schedule Pull Request Comment -->"],
        19: ["/schedule 2022-06-12"],
        20: ["/schedule 2022-06-16", "/schedule 2022-06-14",],
        21: ["/schedule 2022-06-08"],
    };
    const bodies = bodyMappings[pullRequest.number] || ["Sample comment"];
    return bodies.map((body, index) => {
        let id = parseInt(`${pullRequest.number}${index + 1}`, 10)
        return {
            id,
...
export const githubHandlers = [
    ...
    // Get pull request
    rest.get<{}, PullRequestPathParams>(
        githubUrl("/repos/:owner/:repo/pulls/:pull_number"),
        (req, res, ctx) => {
            const pullNumber = parseInt(req.params.pull_number, 10);
            return res(ctx.status(200), ctx.json(pullRequests.find(o => o.number === pullNumber)));
        }
    ),
